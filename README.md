...
Official releases are built from stable `vX.Y.Z` tags by `.github/workflows/release.yml`, which uploads both platforms into a draft and publishes it only after both jobs succeed. Releases are unsigned: macOS gets a deep ad-hoc signature so Gatekeeper shows the ordinary unidentified-developer prompt, and Windows shows SmartScreen.
